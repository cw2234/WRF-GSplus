...
def paint_spectrum_compare(pred_spectrum, gt_spectrum, save_path=None):
    ...
    cax1 = axs[0].pcolormesh(
        theta, r, np.flipud(pred_spectrum).T, cmap="viridis", shading="flat"
    )

    axs[0].axis("off")

    cax2 = axs[1].pcolormesh(
        theta, r, np.flipud(gt_spectrum).T, cmap="viridis", shading="flat"
    )
...
def paint_location(loc_path, save_path):
    ...
    all_loc = np.loadtxt(
        os.path.join(loc_path, "tx_pos.csv"), delimiter=",", skiprows=1
    )
