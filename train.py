...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        R = torch.from_numpy(
            Rotation.from_quat(gateway_orientation).as_matrix()
        ).float()
        tx_pos = tx_pos.cuda()
        viewpoint_cam = generate_new_cam(R, r_o)
        N = gaussians.get_xyz.shape[0]
        time_input = tx_pos.expand(N, -1)
        d_xyz, d_rotation, d_scaling, d_signal = deform.step(
        ...
        if FUSED_SSIM_AVAILABLE:
            ssim_value = fused_ssim(
                image.unsqueeze(0).unsqueeze(0), gt_image.unsqueeze(0).unsqueeze(0)
            )
        else:
        ...
        with torch.no_grad():
            ...
            if iteration in testing_iterations:
                ...
                for test_input, test_label in scene.test_iter:
                    ...
                    ssim_i = ssim(
                        pred_spectrum, gt_spectrum, data_range=1, multichannel=False
                    )
                    logger.info(
                        "Spectrum {:d}, Mean pixel error = {:.6f}; SSIM = {:.6f}".format(
                            save_img_idx, pixel_error, ssim_i
                        )
                    )
                    paint_spectrum_compare(
                        pred_spectrum,
                        gt_spectrum,
                    ...
                    save_img_idx += 1
                    np.savetxt(
                        os.path.join(full_path, "all_ssim.txt"), all_ssim, fmt="%.4f"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[60000, 100000, 200000, 400000, 600000, 800000, 1000000, 1200000],
    )
    parser.add_argument(
        "--save_iterations",
        nargs="+",
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[7000, 30000, 60000, 200000, 300000, 600000, 1200000],
    )
